...
- After every correction: "Should I update AGENTS.md?"
...
- **handle_new_user() trigger**: This function is critical — it runs on every new signup. Never replace it with a simplified version. It must include: email vs OAuth branching, auto-username generation, all NOT NULL columns (`username`, `first_login_completed`, `profile_completed`, `project_access_expires_at`), `is_approved`, and `lecturer_status`. Reference migration 171 (or 145) for the canonical version. Regression here = all signups broken.
