-- Migration 161: Add idempotency guard to project_subscriptions in complete_keepz_payment (PAY-03)
--
-- Problem: The project_subscriptions UPDATE path lacks the "AND status = 'pending'"
--          guard that course and bundle enrollment paths already have. A duplicate
--          Keepz callback could activate a subscription twice and grant an extra month.
-- Fix: Add "AND status = 'pending'" + ROW_COUNT check to match the other paths.
