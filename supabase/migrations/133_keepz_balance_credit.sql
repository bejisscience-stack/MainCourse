-- Migration 133: Add balance credit + referral commission to complete_keepz_payment
--
-- BUG 1: When Keepz payment succeeds, complete_keepz_payment() (migration 129) approves
-- enrollment and creates records, but NEVER calls credit_user_balance().
--
-- BUG 2: approve_enrollment_request (migration 110) uses 'course_sale' as source,
-- but balance_transactions CHECK constraint only allows 'course_purchase'.
-- This also breaks admin manual approval.
--
-- FIX: Add balance credit + referral commission logic to complete_keepz_payment.
-- Also fix approve_enrollment_request and approve_bundle_enrollment_request to use
-- 'course_purchase' (matching the CHECK constraint).
...
BEGIN
      ...
      IF v_course_id IS NOT NULL THEN
        ...
        IF v_balance_already_credited THEN
          ...
          IF FOUND AND v_course.price > 0 THEN
            ...
            ELSE
              PERFORM credit_user_balance(v_course.lecturer_id, v_course.price, 'course_purchase', v_payment.reference_id, 'enrollment_request', 'Course sale (Keepz payment recovery)');
            END IF;

            PERFORM log_payment_event(v_payment.id, p_keepz_order_id, v_payment.user_id, 'balance_recovered',
              jsonb_build_object('course_id', v_course_id, 'amount', v_course.price));
          END IF;
        END IF;
      END IF;
    ...
    ELSIF v_payment.payment_type = 'bundle_enrollment' THEN
      ...
      IF v_balance_already_credited THEN
        ...
        IF FOUND AND v_bundle.price > 0 THEN
          PERFORM credit_user_balance(v_bundle.lecturer_id, v_bundle.price, 'course_purchase', v_payment.reference_id, 'bundle_enrollment_request', 'Bundle sale (Keepz payment recovery)');
          PERFORM log_payment_event(v_payment.id, p_keepz_order_id, v_payment.user_id, 'bundle_balance_recovered',
            jsonb_build_object('bundle_id', v_bundle.id, 'amount', v_bundle.price));
        END IF;
      END IF;
  ...
  IF v_payment.payment_type = 'course_enrollment' THEN
    ...
    IF FOUND AND v_course.price > 0 THEN
      ...
      IF FOUND AND v_course.referral_commission_percentage > 0 THEN
        -- Split payment between referrer and lecturer
        v_commission := (v_course.referral_commission_percentage::DECIMAL / 100) * v_course.price;
        v_referrer_amount := v_commission;
        v_lecturer_amount := v_course.price - v_commission;
        ...
        PERFORM credit_user_balance(v_course.lecturer_id, v_lecturer_amount, 'course_purchase', v_payment.reference_id, 'enrollment_request', 'Course sale from Keepz payment (after referral commission)');

        PERFORM log_payment_event(v_payment.id, p_keepz_order_id, v_payment.user_id, 'balance_credited_with_referral',
          jsonb_build_object('course_id', v_course_id, 'lecturer_amount', v_lecturer_amount, 'referrer_amount', v_referrer_amount, 'referrer_id', v_referral.referrer_id));
      ELSE
        -- No referral or 0% commission — lecturer gets full price
        PERFORM credit_user_balance(v_course.lecturer_id, v_course.price, 'course_purchase', v_payment.reference_id, 'enrollment_request', 'Course sale from Keepz payment');

        PERFORM log_payment_event(v_payment.id, p_keepz_order_id, v_payment.user_id, 'balance_credited',
          jsonb_build_object('course_id', v_course_id, 'amount', v_course.price, 'lecturer_id', v_course.lecturer_id));
      END IF;
    END IF;
  ...
  ELSIF v_payment.payment_type = 'bundle_enrollment' THEN
    ...
    -- =====================================================================
    -- BALANCE CREDIT: credit lecturer for bundle sale
    -- =====================================================================
    SELECT cb.* INTO v_bundle FROM course_bundles cb
      JOIN bundle_enrollment_requests ber ON ber.bundle_id = cb.id
      WHERE ber.id = v_payment.reference_id;
    ...
    IF FOUND AND v_bundle.price > 0 THEN
      PERFORM credit_user_balance(v_bundle.lecturer_id, v_bundle.price, 'course_purchase', v_payment.reference_id, 'bundle_enrollment_request', 'Bundle sale from Keepz payment');

      PERFORM log_payment_event(v_payment.id, p_keepz_order_id, v_payment.user_id, 'bundle_balance_credited',
        jsonb_build_object('bundle_id', v_bundle.id, 'amount', v_bundle.price, 'lecturer_id', v_bundle.lecturer_id));
    END IF;
...
-- =========================================================================
-- FIX: approve_enrollment_request — change 'course_sale' → 'course_purchase'
-- to match balance_transactions CHECK constraint
-- =========================================================================
...
-- =========================================================================
-- FIX: approve_bundle_enrollment_request — change 'bundle_sale' → 'course_purchase'
-- =========================================================================
