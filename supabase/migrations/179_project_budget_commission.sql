-- Migration 179: Add 20% platform commission to project budgets
--
-- Business logic:
--   Lecturer sets budget (e.g., ₾100).
--   Lecturer pays budget × 1.20 (e.g., ₾120). The 20% is platform commission.
--   Project activates with budget = ₾100 (base amount for spending).
--   Platform keeps ₾20, recorded in keepz_payments.platform_commission.
--   No balance credit — project budget is spent through project_submissions.
--
-- Changes:
--   1. Redefine complete_keepz_payment() to calculate 20% commission for project_budget

-- ============================================================================
-- 1. Redefine complete_keepz_payment() — add project budget commission (20%)
--    Source: migration 178 (the current version)
--    Only the project_budget sections are changed.
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_payment.payment_type = 'bundle_enrollment' THEN
    ...
    END IF;
  ELSIF v_payment.payment_type = 'project_budget' THEN
    -- Project budget: 20% commission on the base budget
    SELECT budget INTO v_project_budget FROM projects WHERE id = v_payment.reference_id;
    IF v_project_budget IS NOT NULL AND v_project_budget > 0 THEN
      v_platform_commission := ROUND(v_project_budget * 0.20, 2);
    ELSE
