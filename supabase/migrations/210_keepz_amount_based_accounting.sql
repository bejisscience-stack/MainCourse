-- Migration 210: Account commissions/credits from the actual paid amount
--
-- Why: with the 12-hour welcome discount (migration 209), users outside their
-- discount window pay courses.original_price (or course_bundles.original_price)
-- instead of courses.price. The previous accounting logic in migration 179
-- still computed platform commission, referral commission, and lecturer
-- balance from courses.price / course_bundles.price, so:
--   * lecturer was under-credited when a user paid the (higher) original_price;
--   * the delta (original_price − price minus its share of fees) was silently
--     retained by the platform with no entry to explain it;
--   * the converse holds for in-window users — historically users always paid
--     courses.price, so this migration is a NO-OP for the in-window case.
--
-- What changes: course_enrollment and bundle_enrollment paths now compute
--   v_platform_commission, v_commission (referral), and v_lecturer_amount
--   from v_payment.amount (the amount actually charged). project_subscription
--   and project_budget are untouched — they have their own pricing model.
--
-- Source: migration 179 (the current version). Only the lines noted above
-- change. Function signature, error handling, and ordering are preserved
-- verbatim so the recovery, idempotency, and exception semantics are stable.
...
BEGIN
  ...
  IF v_payment.status = 'success' THEN
      ...
      IF v_course_id IS NOT NULL THEN
        ...
        IF v_balance_already_credited THEN
          ...
          IF FOUND AND v_payment.amount > 0 THEN
            -- Platform commission = 3% of the actual paid amount
            ...
            IF FOUND AND v_course.referral_commission_percentage > 0 THEN
              -- Referral commission scales with the actual paid amount
    ...
    ELSIF v_payment.payment_type = 'project_budget' THEN
      ...
      -- Recovery: record platform commission if not already set
  ...
  -- Calculate platform commission based on payment type. For courses and
  -- bundles, this is now 3% of the actual paid amount, so the rate floats
  -- with the welcome discount (in-window: based on `price`, out-of-window:
  -- based on `original_price`). For project_budget the platform retains its
  -- existing 20% of the project's base budget.
  ...
  ELSIF v_payment.payment_type = 'bundle_enrollment' THEN
    ...
    END IF;
  ELSIF v_payment.payment_type = 'project_budget' THEN
    -- Project budget: 20% commission on the base budget (unchanged)
    SELECT budget INTO v_project_budget FROM projects WHERE id = v_payment.reference_id;
    IF v_project_budget IS NOT NULL AND v_project_budget > 0 THEN
      v_platform_commission := ROUND(v_project_budget * 0.20, 2);
    ELSE
  ...
  BEGIN
    ...
    IF v_payment.payment_type = 'course_enrollment' THEN
      ...
      -- All amounts now derived from v_payment.amount (the price actually paid)
      -- so the welcome discount and out-of-window pricing flow correctly to
      -- the lecturer/referrer/platform split.
      -- =====================================================================
      SELECT * INTO v_course FROM courses WHERE id = v_course_id;
    ...
    ELSIF v_payment.payment_type = 'bundle_enrollment' THEN
      ...
      -- =====================================================================
      -- BALANCE CREDIT: lecturer credit derived from v_payment.amount
      -- =====================================================================
      SELECT cb.* INTO v_bundle FROM course_bundles cb
        JOIN bundle_enrollment_requests ber ON ber.bundle_id = cb.id
        WHERE ber.id = v_payment.reference_id;
