-- Migration 148: Fix double-credit vulnerability in complete_keepz_payment (SEC-05)
--
-- Problem: Concurrent callbacks could approve already-approved enrollment
--          requests and double-credit lecturer balances because the UPDATE
--          had no status guard.
-- Fix: Add "AND status = 'pending'" to enrollment approval UPDATEs and
--       check ROW_COUNT — if 0 rows affected, return early as already completed.
