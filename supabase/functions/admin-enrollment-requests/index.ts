...
Deno.serve(async (req: Request) => {
  console.warn(
    `[deprecated] edge fn 'admin-enrollment-requests' invoked — caller=${req.headers.get("user-agent") ?? "unknown"} origin=${req.headers.get("origin") ?? "unknown"}`,
  );
  ...
  try {
    ...
    console.log(
      "[Admin API] Fetching requests, filter:",
      filterStatus || "all",
    );
