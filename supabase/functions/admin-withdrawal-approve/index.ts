...
Deno.serve(async (req: Request) => {
  console.warn(
    `[deprecated] edge fn 'admin-withdrawal-approve' invoked — caller=${req.headers.get("user-agent") ?? "unknown"} origin=${req.headers.get("origin") ?? "unknown"}`,
  );
  ...
  try {
    ...
    console.log(
      "[Approve Withdrawal API] Request approved successfully:",
      requestId,
    );
    ...
    try {
      ...
      if (notificationError) {
        console.error(
          "[Approve Withdrawal API] Error creating notification:",
          notificationError,
        );
      } else {
        console.log(
          "[Approve Withdrawal API] Notification created for user:",
          withdrawalRequest.user_id,
        );
      }
    } catch (notifError) {
      console.error(
        "[Approve Withdrawal API] Exception creating notification:",
        notifError,
      );
    }
    ...
    try {
      ...
      console.error(
        "[Approve Withdrawal API] Error sending email:",
        emailError,
      );
    }
