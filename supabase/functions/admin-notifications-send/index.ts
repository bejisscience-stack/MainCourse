  ...
  try {
    ...
    if (!title?.en || !title?.ge) {
      return errorResponse(
        "Title in both English and Georgian is required",
        400,
      );
    }
    ...
    if (!message?.en || !message?.ge) {
      return errorResponse(
        "Message in both English and Georgian is required",
        400,
      );
    }
    ...
    switch (target_type) {
      case "all": {
        ...
        if (allError) {
          console.error(
            "[Admin Notifications API] Error fetching all users:",
            allError,
          );
      ...
      case "role": {
        if (!target_role) {
          return errorResponse(
            'target_role is required when target_type is "role"',
            400,
          );
        }

        const { data: roleUserIds, error: roleError } =
          await serviceSupabase.rpc("get_user_ids_by_role", {
            p_role: target_role,
          });

        if (roleError) {
          console.error(
            "[Admin Notifications API] Error fetching users by role:",
            roleError,
          );
          return jsonResponse({ error: "Failed to fetch users by role" }, 500);
        }
      ...
      case "course": {
        if (!target_course_id) {
          return errorResponse(
            'target_course_id is required when target_type is "course"',
            400,
          );
        }

        const { data: courseUserIds, error: courseError } =
          await serviceSupabase.rpc("get_enrolled_user_ids", {
            p_course_id: target_course_id,
          });

        if (courseError) {
          console.error(
            "[Admin Notifications API] Error fetching enrolled users:",
            courseError,
          );
          return jsonResponse({ error: "Failed to fetch enrolled users" }, 500);
        }
      ...
      case "specific": {
        if (!target_user_ids || target_user_ids.length === 0) {
          return errorResponse(
            'target_user_ids is required when target_type is "specific"',
            400,
          );
        }
    ...
    const { data: count, error: sendError } = await serviceSupabase.rpc(
      "send_bulk_notifications",
      {
        ...
        p_title_en: sanitizeText(title.en),
        p_title_ge: sanitizeText(title.ge),
        p_message_en: sanitizeText(message.en),
        p_message_ge: sanitizeText(message.ge),
        p_metadata: {},
    ...
    if (sendError) {
      console.error(
        "[Admin Notifications API] Error sending notifications:",
        sendError,
      );
    ...
    console.log(
      "[Admin Notifications API] Successfully sent",
      count,
      "notifications",
    );
