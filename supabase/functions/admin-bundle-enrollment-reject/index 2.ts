...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (enrollmentRequest?.user_id) {
      const bundleTitle = (enrollmentRequest.course_bundles as { title?: string } | null)?.title || 'Unknown Bundle'
      ...
      try {
        ...
        if (userProfile?.email) {
          await sendBundleEnrollmentRejectedEmail(userProfile.email, bundleTitle, reason)
