...
Deno.serve(async (req: Request) => {
  console.warn(
    `[deprecated] edge fn 'enrollment-requests' invoked — caller=${req.headers.get("user-agent") ?? "unknown"} origin=${req.headers.get("origin") ?? "unknown"}`,
  );
  ...
  try {
    ...
    const { data: enrollmentRequest, error: insertError } = await supabase
      ...
      .insert({
        ...
        status: "pending",
        payment_screenshots:
          formattedScreenshots.length > 0 ? formattedScreenshots : [],
    ...
    if (enrollmentRequest.id) {
      try {
        ...
        if (!referralProcessed) {
          const { error: signupRefError } = await supabase.rpc(
