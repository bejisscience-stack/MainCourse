...
Deno.serve(async (req: Request) => {
  console.warn(
    `[deprecated] edge fn 'admin-bundle-enrollment-requests' invoked — caller=${req.headers.get("user-agent") ?? "unknown"} origin=${req.headers.get("origin") ?? "unknown"}`,
  );
  ...
  try {
    ...
    console.log(
      "[Admin Bundle API] Fetching requests, filter:",
      filterStatus || "all",
    );
