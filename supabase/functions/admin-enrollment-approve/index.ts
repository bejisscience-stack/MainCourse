...
Deno.serve(async (req: Request) => {
  console.warn(
    `[deprecated] edge fn 'admin-enrollment-approve' invoked — caller=${req.headers.get("user-agent") ?? "unknown"} origin=${req.headers.get("origin") ?? "unknown"}`,
  );
    ...
    if (verifyError) {
      ...
      if (updatedRequest?.user_id) {
        ...
        try {
          ...
          if (userEmail) {
            await sendEnrollmentApprovedEmail(userEmail, courseTitle);
