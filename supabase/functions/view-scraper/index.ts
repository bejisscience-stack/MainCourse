...
function extractTikTokMetrics(result: any): ExtractedMetrics | null {
  ...
  return {
    viewCount,
    likeCount: firstNumber(
      result.diggCount,
      result.likesCount,
      result.stats?.diggCount,
      result.statsV2?.diggCount,
    ),
    commentCount: firstNumber(
      result.commentCount,
      result.stats?.commentCount,
      result.statsV2?.commentCount,
    ),
    shareCount: firstNumber(
      result.shareCount,
      result.stats?.shareCount,
      result.statsV2?.shareCount,
    ),
    saveCount: firstNumber(
      result.collectCount,
      result.stats?.collectCount,
      result.statsV2?.collectCount,
    ),
  };
}
...
function debugKeys(result: any): string {
  ...
  if (keys.length > 30)
    return keys.slice(0, 30).join(", ") + `... (${keys.length} total)`;
  return keys.join(", ");
}
...
function timingSafeSecretEqual(a: string, b: string): boolean {
  try {
    ...
    // Different lengths or unsupported — reject
    return false;
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (
      scraperSecret &&
      expectedSecret &&
      timingSafeSecretEqual(scraperSecret, expectedSecret)
    ) {
    ...
    for (const sub of submissions || []) {
      ...
      if (sub.platform_links && typeof sub.platform_links === "object") {
        for (const [, url] of Object.entries(
          sub.platform_links as Record<string, string>,
        )) {
      ...
      if (sub.video_url) {
        ...
        if (platform) {
          const alreadyHasUrl = videoEntries.some(
            (e) =>
              e.submissionId === sub.id &&
              normalizeUrl(e.originalUrl) === normalizeUrl(sub.video_url!),
          );
          if (!alreadyHasUrl) {
            videoEntries.push({
              submissionId: sub.id,
    ...
    const instagramEntries = videoEntries.filter(
    ...
    if (tiktokEntries.length > 0) {
      try {
        ...
        if (!actorResponse.ok) {
          throw new Error(
            `Apify TikTok actor returned ${actorResponse.status}`,
          );
        }
        ...
        if (datasetId) {
          ...
          const singleMatch =
            tiktokEntries.length === 1 && results.length === 1;

          // Match back to entries
          for (const entry of tiktokEntries) {
            const result =
              resultMap.get(normalizeUrl(entry.originalUrl)) ||
              (singleMatch ? results[0] : null);
    ...
    if (instagramEntries.length > 0) {
      try {
        ...
        if (!actorResponse.ok) {
          throw new Error(
            `Apify Instagram actor returned ${actorResponse.status}`,
          );
        }
        ...
        if (datasetId) {
          ...
          const singleMatchIg =
            instagramEntries.length === 1 && results.length === 1;

          for (const entry of instagramEntries) {
            const result =
              resultMap.get(normalizeUrl(entry.originalUrl)) ||
              (singleMatchIg ? results[0] : null);
    ...
    // Update run record
    const finalStatus =
      failCount === videoEntries.length ? "failed" : "completed";
    await adminClient
      .from("view_scrape_runs")
      .update({
        status: finalStatus,
