...
Deno.serve(async (req: Request) => {
  console.warn(
    `[deprecated] edge fn 'notification-read' invoked — caller=${req.headers.get("user-agent") ?? "unknown"} origin=${req.headers.get("origin") ?? "unknown"}`,
  );
  ...
  try {
    ...
    if (!notificationId) {
      return errorResponse(
        "Notification ID is required (in body as notificationId or query param)",
        400,
        cors,
      );
    }
