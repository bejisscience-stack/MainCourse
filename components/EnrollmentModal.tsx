...
export default function EnrollmentModal({
  ...
  const { cards, isLoading: cardsLoading, deleteCard, mutate: mutateSavedCards } = useSavedCards();
  ...
  const [tokenPaymentStatus, setTokenPaymentStatus] = useState<null | 'processing' | 'success' | 'failed'>(null);
  ...
  useEffect(() => {
    ...
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !isSubmitting && tokenPaymentStatus !== 'processing') onClose();
    };
  ...
  const pollPaymentStatus = useCallback(async (paymentId: string, token: string) => {
    let attempts = 0;
  ...
  const handlePayWithSavedCard = useCallback(async (savedCardId: string) => {
    setError(null);
  ...
  }, [course.id, enrollmentMode, referralCode, isReEnrollment, pollPaymentStatus]);
  ...
  const isPayDisabled = isSubmitting || referralValidation === 'validating' || tokenPaymentStatus === 'processing';
  ...
  const modalContent = (
    <div
      ...
      onClick={() => { if (!isSubmitting && tokenPaymentStatus !== 'processing') onClose(); }}
    >
      <div
      ...
      >
        ...
        {tokenPaymentStatus !== 'processing' && tokenPaymentStatus !== 'success' && (
          <>
            {/* Referral Code */}
              ...
              <div className="p-6 pt-5 pb-2">
                ...
                <div className="space-y-2">
                  {cards.map((card) => (
                    <div
                      key={card.id}
                      className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-navy-700 bg-gray-50 dark:bg-navy-800/50"
                    >
                      {/* Card info */}
