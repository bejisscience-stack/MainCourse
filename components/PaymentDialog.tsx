...
export default function PaymentDialog({
  ...
  const [uploadedImages, setUploadedImages] = useState<
    Array<{ file: File; preview: string; url?: string }>
  >([]);
  ...
  const formattedPrice = useMemo(
    () => formatPriceInGel(calculateStudentPrice(course.price)),
    [course.price],
  );
    ...
    try {
      ...
      for (const imageData of uploadedImages) {
        ...
        if (uploadError) {
          ...
          throw new Error(
            `Failed to upload ${imageData.file.name}: ${uploadError.message}`,
          );
        }
      ...
      >
        ...
        <div className="w-full max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 py-10 md:py-20">
          ...
          <div className="bg-charcoal-50/50 dark:bg-navy-700/50 rounded-2xl p-6 space-y-4">
            <div>
              <h3 className="text-xl md:text-2xl font-semibold text-charcoal-950 dark:text-white">
                {course.title}
              </h3>
              {course.description && (
                <p className="text-base text-charcoal-600 dark:text-gray-400 mt-2">
                  {course.description}
                </p>
              )}
            </div>
            <div className="flex items-center justify-between pt-4 border-t border-charcoal-200/50 dark:border-navy-600/50">
              <div>
                <p className="text-sm md:text-base text-charcoal-500 dark:text-gray-400 mb-1">
                  {t("payment.creator")}
                </p>
