      ...
      >
        ...
        <div className="relative w-full h-32 bg-gradient-to-br from-emerald-50 via-white to-charcoal-50/30 dark:from-emerald-500/10 dark:via-navy-800 dark:to-navy-700/30 overflow-hidden cursor-pointer group">
          ...
          <div className="absolute bottom-2 left-2 bg-white/80 dark:bg-navy-800/80 backdrop-blur-sm px-2 py-1 rounded-full flex items-center space-x-1.5 border border-charcoal-100/50 dark:border-navy-700/50 z-20 shadow-soft">
            ...
            <span className="text-charcoal-600 dark:text-gray-300 text-[10px] font-medium">
              {course.creator}
            </span>
          </div>
          ...
          <div
            className={`absolute top-2 right-2 ${courseTypeConfig.bgColor} backdrop-blur-sm px-2.5 py-1.5 rounded-lg flex items-center space-x-1.5 border ${courseTypeConfig.borderColor} z-20 shadow-md`}
          >
            <span className={`${courseTypeConfig.textColor}`}>
              {courseTypeConfig.icon}
            </span>
            <span
              className={`${courseTypeConfig.textColor} text-xs font-semibold`}
            >
              {course.course_type}
            </span>
          </div>
          ...
          {!isEnrolled && discountPercent !== null && (
            <div className="absolute top-2 left-2 bg-red-500 text-white px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-md sm:rounded-lg z-20 shadow-md text-[11px] sm:text-xs font-bold tracking-wide uppercase">
              -{discountPercent}%
              <span className="hidden sm:inline"> {t("courses.sale")}</span>
            </div>
          )}
          ...
          {isEnrolled && daysRemaining !== null && (
            <div
              className={`absolute top-2 left-2 backdrop-blur-sm px-2.5 py-1.5 rounded-lg z-20 shadow-md text-xs font-semibold ${
                isExpired
            ...
            >
              {isExpired
                ...
                : t("enrollment.daysLeft", { days: daysRemaining })}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col p-5">
          <div className="flex-1 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`${courseTypeConfig.bgColor} ${courseTypeConfig.textColor} border ${courseTypeConfig.borderColor} text-xs font-semibold px-2.5 py-1 rounded-md flex items-center space-x-1.5`}
              >
                {courseTypeConfig.icon}
