...
export default function CoursesCarousel() {
  ...
  // Responsive slidesPerView: 3 on lg+, 2 on md-lg
  useEffect(() => {
    const lgQuery = window.matchMedia("(min-width: 1024px)");
    const handleChange = () => setSlidesPerView(lgQuery.matches ? 3 : 2);
    handleChange();
  ...
  // Clamp currentIndex when courses or slidesPerView change
  useEffect(() => {
    const max = Math.max(0, courses.length - slidesPerView);
    if (currentIndex > max) setCurrentIndex(max);
  }, [courses.length, slidesPerView, currentIndex]);

  // Auto-rotation every 4 seconds when not hovered
  useEffect(() => {
    if (courses.length <= slidesPerView) return;
    const maxIdx = Math.max(0, courses.length - slidesPerView);
  ...
  }, [isHovered, courses.length, slidesPerView]);
  ...
  const handleEnroll = useCallback(
    async (courseId: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            result.error || "Failed to create enrollment request",
          );
        }
  ...
  const showArrows = courses.length > slidesPerView;
  const translateX = -safeCurrentIndex * stepSize;
    ...
    <section className="px-4 sm:px-6 lg:px-8 pb-24 md:pb-32">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative">
              ...
              <div className="flex justify-center mt-2">
                ...
                >
                  {showAllMobile ? (
                    <>
                      <span>
                        {translationsReady ? t("common.showLess") : "Show Less"}
                      </span>
                      <svg
                    ...
                    <>
                      <span>
                        {translationsReady ? t("common.showMore") : "Show More"}
                      </span>
                      <svg
                        className="w-4 h-4"
                        fill="none"
          ...
          {/* Desktop View: Flat Sliding Carousel */}
