...
function AdminNotificationSender() {
  ...
  const [emailTarget, setEmailTarget] = useState<
    "profiles" | "coming_soon" | "both" | "specific"
  >("profiles");
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const parts: string[] = [];
      if (data.in_app_count > 0)
        parts.push(`${data.in_app_count} in-app notification(s)`);
      if (data.email_count > 0) parts.push(`${data.email_count} email(s)`);
      if (data.email_failed_count > 0)
        parts.push(`${data.email_failed_count} email(s) failed`);

      if (
        data.email_failed_count > 0 &&
        data.email_count === 0 &&
        data.in_app_count === 0
      ) {
        throw new Error(
          `All emails failed to send. Error: ${data.email_error || "Unknown error"}`,
        );
      }
      ...
      if (data.email_failed_count > 0) {
        ...
        setSuccessMessage(
          `Partially sent: ${parts.join(", ")}. Error: ${data.email_error || "Unknown"}`,
        );
      } else {
        setIsPartialSuccess(false);
        setSuccessMessage(
          `Successfully sent: ${parts.join(", ") || "No notifications sent"}`,
        );
      }
    ...
    switch (targetType) {
      ...
      case "course":
        ...
        return course
          ...
          : "Users enrolled in selected course";
      case "specific":
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {showInAppTargeting && (
                  <div className="bg-blue-50 rounded-lg p-3">
                    <p className="text-sm text-blue-800">
                      <span className="font-medium">In-App Target:</span>{" "}
                ...
                {showEmailTargeting && (
                  <div className="bg-purple-50 rounded-lg p-3">
                    <p className="text-sm text-purple-800">
                      <span className="font-medium">Email Target:</span>{" "}
                      {getEmailTargetDescription()}
                    </p>
                  </div>
                )}
              </div>
