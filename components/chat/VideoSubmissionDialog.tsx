...
export default function VideoSubmissionDialog({
  ...
  const handlePlatformLinkChange = useCallback(
      ...
      if (link && link.length > 0) {
        try {
          ...
          if (!validatePlatformUrl(platform, link)) {
            ...
            newErrors[`platform_${platform}`] = t(
              "videoSubmission.invalidPlatformUrl",
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        setErrors({
          submit: error.message || "Failed to submit video. Please try again.",
        });
      } finally {
        ...
        >
          ...
          <div>
            <h2 className="text-2xl font-bold text-white mb-2">
              {t("videoSubmission.submitYourVideo")}
            </h2>
            <p className="text-gray-400 text-sm">
              {t("videoSubmission.submitVideoLinks")}
            </p>
          </div>
                ...
                {platforms.map((platform) => (
                  <div key={platform}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handlePlatformLinkChange(platform, e.target.value)
                      }
