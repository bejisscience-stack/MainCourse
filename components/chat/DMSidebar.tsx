...
export default function DMSidebar({
  ...
  // Realtime: refresh data on changes
  useRealtimeFriends({
  ...
  // Friends that don't have an active DM conversation
      ...
      <div className="flex-1 overflow-y-auto px-2.5 py-3 chat-scrollbar">
        ...
        {conversations.length === 0 && friendships.length === 0 && totalPendingRequests === 0 && (
          <div className="flex flex-col items-center justify-center py-8 text-gray-500">
            ...
            <p className="text-sm font-medium text-gray-300 mb-1">No friends yet</p>
            <p className="text-xs text-gray-500 mb-3">Add friends to start messaging</p>
            <button
              onClick={onAddFriend}
              className="text-emerald-200 text-xs font-medium px-4 py-2 bg-emerald-500/15 border border-emerald-500/40 rounded-lg hover:bg-emerald-500/25 transition-colors"
            >
              Add Friend
            </button>
          </div>
