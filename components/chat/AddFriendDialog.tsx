...
export default function AddFriendDialog({ isOpen, onClose, currentUserId }: AddFriendDialogProps) {
  ...
  const { sent, sendFriendRequest, isSubmitting } = useFriendRequests(currentUserId);
  ...
  const getStatus = (userId: string): 'friend' | 'pending' | 'available' => {
    if (friendIds.has(userId)) return 'friend';
    if (pendingSentIds.has(userId)) return 'pending';
    return 'available';
  };
          ...
          {results.map((user) => {
            ...
            return (
              ...
              >
                ...
                {/* Action */}
                {status === 'friend' && (
                  <span className="text-emerald-300 text-xs font-medium px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
                    Friends
                  </span>
                )}
                {status === 'pending' && (
