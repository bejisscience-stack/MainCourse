      ...
      const channel = supabase
        ...
        .on(
          ...
          async (payload) => {
            ...
            const [actualUsername, replyPreview, attachments] =
              await Promise.all([
                cachedUsername === "User"
                  ? fetchAndCacheProfile(messageData.user_id)
                  : Promise.resolve(cachedUsername),
                messageData.reply_to_id
                  ? fetchReplyPreview(messageData.reply_to_id)
                  : Promise.resolve(undefined),
                fetchAttachments(messageData.id),
              ]);

            if (
              actualUsername !== cachedUsername ||
              replyPreview ||
              attachments
            ) {
