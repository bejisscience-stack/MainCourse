...
function CoursesPageContent() {
  ...
  const [showBundleEnrollmentWizard, setShowBundleEnrollmentWizard] =
    useState(false);
  ...
  // Self-heal stale payments where Keepz callback never arrived
  usePaymentRecovery(user?.id || null, mutateEnrollments);

  const [bundles, setBundles] = useState<any[]>([]);
  const [enrolledBundleIds, setEnrolledBundleIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  useRealtimeBundleEnrollmentRequests({
    ...
    onRequestApproved: (request) => {
      const bundleName =
        bundles.find((b) => b.id === request.bundle_id)?.title || "Bundle";
      toast.success(
        t("enrollment.enrollmentApproved") ||
          `Your enrollment for "${bundleName}" has been approved!`,
        { duration: 5000 },
      );
    },
    onRequestRejected: (request) => {
      ...
      toast.error(
        t("enrollment.enrollmentRejected") ||
          `Your enrollment request for "${bundleName}" was rejected.`,
        { duration: 5000 },
      );
    },
  });
  ...
  const { enrolledCourses, availableCourses } = useMemo(() => {
    const enrolled = filteredCourses.filter((course) =>
  ...
  const handleEnroll = useCallback(
    async (courseId: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            result.error || t("courses.errorRequestingEnrollment"),
          );
        }
                  ...
                  type === "All"
                    ...
                    : type === "Editing"
                      ...
                      : type === "Content Creation"
                        ? "filterContentCreation"
                        : "filterWebsiteCreation";
                return (
                  <button
                    key={type}
          ...
          {isLoading && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white dark:bg-navy-800 rounded-lg overflow-hidden shadow-md border border-gray-100 dark:border-navy-700 animate-pulse"
                >
            ...
            <section className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {enrolledCourses.map((course) => {
                  ...
                  const shouldShowEnroll =
                    !isOwnCourse && userRole !== "lecturer";
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {bundles.map((bundle) => {
                  ...
                  return (
                    <div
                      key={bundle.id}
                      className="h-full flex flex-col bg-white dark:bg-navy-800 border-2 border-purple-200 dark:border-purple-700/50 rounded-2xl overflow-hidden shadow-soft hover:shadow-soft-lg transition-all duration-200"
                    >
                      ...
                      <div className="bg-gradient-to-r from-purple-500/10 to-emerald-500/10 dark:from-purple-500/20 dark:to-emerald-500/20 p-5 border-b border-purple-100 dark:border-purple-800/50">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide bg-purple-100 dark:bg-purple-500/30 px-2.5 py-1 rounded-md">
                            {t("courses.bundle")}
                          </span>
                        ...
                        <h3 className="text-lg font-bold text-charcoal-950 dark:text-white line-clamp-2">
                          {bundle.title}
                        </h3>
                        {bundle.description && (
                          <p className="text-sm text-charcoal-600 dark:text-gray-400 line-clamp-2 mt-2">
                            {bundle.description}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex-1 p-5">
                        <p className="text-xs font-medium text-charcoal-500 dark:text-gray-500 mb-3">
                          {t("courses.includesCourses", {
                        ...
                        <div className="space-y-2 max-h-28 overflow-y-auto">
                          {bundleCourses.map((course: any, idx: number) => (
                            <div
                              key={idx}
                              className="flex items-center text-sm text-charcoal-700 dark:text-gray-300"
                            >
                              <svg
                              ...
                              <span className="truncate">
                                {course?.title || t("courses.unknownCourse")}
                              </span>
                            </div>
                          ))}
                        </div>
              ...
              {availableCourses.length === 0 && filteredCourses.length === 0 ? (
                <div className="text-center py-16 bg-white/50 dark:bg-navy-800/50 rounded-2xl border border-charcoal-100/50 dark:border-navy-700/50">
                  ...
                  <p className="text-charcoal-600 dark:text-gray-400 text-lg mb-2">
                    {searchQuery
                      ? t("courses.noCoursesFoundForSearch", {
                          query: searchQuery,
                        })
                      : filter !== "All"
                    ...
                    {availableCourses.map((course) => {
                      ...
                      return (
                        <CourseEnrollmentCard
                          ...
                          userId={user?.id || null}
                          initialReferralCode={
                            isSelectedCourse ? urlReferralCode : null
                          }
