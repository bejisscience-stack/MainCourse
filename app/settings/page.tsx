...
export default function SettingsPage() {
  ...
  const updateProfileRpc = async (data: {
    username?: string;
    ...
    const { data: result, error } = await supabase.rpc("update_own_profile", {
      ...
      new_avatar_url:
        data.avatar_url === null ? "__REMOVE__" : (data.avatar_url ?? null),
    });
    ...
    if (error) {
      if (error.message?.includes("username_taken")) {
        throw {
          code: "username_taken",
          message: t("settings.usernameAlreadyTaken"),
        };
      }
  ...
  const handleBankAccountUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBankAccountError(
        err.message ||
          t("settings.failedToUpdateBankAccount") ||
  ...
  const handleWithdrawalRequest = async (e: React.FormEvent) => {
    ...
    if (amount > balance) {
      setWithdrawalError(
        t("settings.insufficientBalance") || "Insufficient balance",
      );
      return;
    }
    ...
    if (!accountToUse) {
      setWithdrawalError(
        t("settings.bankAccountRequired") ||
          "Please enter a valid bank account number first",
      );
      return;
    }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                >
                  ...
                  <button
                    ...
                    disabled={
                      isUpdatingProfile ||
                      profileUsername.trim() === (userProfile?.username || "")
                    }
              ...
              {loadingReferralCode ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-charcoal-50 dark:bg-navy-700/50 border border-charcoal-200 dark:border-navy-600 rounded-xl px-4 py-3">
                    <p className="text-2xl font-mono font-bold text-charcoal-950 dark:text-white tracking-wider">
                      {referralCode}
                    </p>
                  </div>
                  <button
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4">
                  <p className="text-sm text-yellow-800 dark:text-yellow-300">
                    {t("settings.referralCodeNotAvailable")}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white dark:bg-navy-800 border border-charcoal-100/50 dark:border-navy-700/50 rounded-3xl p-6 shadow-soft">
              ...
              <p className="text-sm text-charcoal-600 dark:text-gray-400 mb-6">
                {t("settings.balanceDescription") ||
                  "View your earnings from referrals and course purchases, and manage withdrawals."}
              </p>
                  ...
                  <div className="border-t border-charcoal-200 dark:border-navy-600 pt-6">
                    ...
                    >
                      <div>
                        ...
                        />
                        <p className="text-xs text-charcoal-500 dark:text-gray-400 mt-1">
                          {t("settings.bankAccountHint") ||
                            "Enter your Georgian bank account number (IBAN format)"}
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          bankAccountInput === bankAccountNumber
                        }
                        className="px-6 py-3 bg-charcoal-950 dark:bg-emerald-500 text-white font-semibold rounded-xl hover:bg-charcoal-800 dark:hover:bg-emerald-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isUpdatingBankAccount
                          ? t("settings.saving") || "Saving..."
                          : t("settings.saveBankAccount") ||
                            "Save Bank Account"}
                      </button>
                    </form>
                  </div>
                  ...
                  <div className="border-t border-charcoal-200 dark:border-navy-600 pt-6">
                    ...
                    {pendingWithdrawal > 0 && (
                      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mb-4">
                        <p className="text-sm text-blue-800 dark:text-blue-300">
                          {t("settings.pendingWithdrawalMessage", {
                            amount: pendingWithdrawal.toFixed(2),
                          })}
                        </p>
                      </div>
                    )}
                    ...
                    {showWithdrawalForm &&
                      ...
                      pendingWithdrawal === 0 && (
                        <form
                          ...
                          className="space-y-4 bg-charcoal-50 dark:bg-navy-700/30 rounded-xl p-4"
                        >
                          <div>
                            <label className="block text-sm font-medium text-charcoal-700 dark:text-gray-300 mb-2">
                              {t("settings.withdrawalAmount") ||
                                "Withdrawal Amount (₾)"}
                            </label>
                            <input
                              type="number"
                          ...
                          <div className="flex gap-3">
                            ...
                            >
                              {isRequestingWithdrawal
                                ? t("settings.processing") || "Processing..."
                                : t("settings.submitWithdrawal") ||
                    ...
                    {withdrawalSuccess && (
                      <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-300 px-4 py-3 rounded-xl text-sm mt-4">
                        {t("settings.withdrawalRequestSubmitted") ||
                        ...
                        <div className="mt-4 space-y-3 max-h-80 overflow-y-auto">
                          {transactions.map((transaction) => (
                            ...
                            >
                                  ...
                                  <p className="text-sm font-medium text-charcoal-950 dark:text-white">
                                    {transaction.source ===
                                      ...
                                      (t("settings.referralCommission") ||
                                        "Referral Commission")}
                                    {transaction.source === "course_purchase" &&
                                      (t("settings.coursePurchase") ||
                                        "Course Purchase")}
                              ...
                              <p
                                className={`text-sm font-bold ${transaction.transaction_type === "credit" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
                              >
                      ...
                      <div className="space-y-3 max-h-60 overflow-y-auto">
                        {withdrawalRequests.map((request) => (
                          ...
                          >
                            ...
                            >
                              {request.status === "pending" &&
                                (t("settings.statusPending") || "Pending")}
                              {request.status === "completed" &&
                                (t("settings.statusCompleted") || "Completed")}
                              {request.status === "rejected" &&
                                (t("settings.statusRejected") || "Rejected")}
                              {request.status === "approved" &&
                                (t("settings.statusApproved") || "Approved")}
                            </span>
                          </div>
                        ))}
                      </div>
            ...
            {referralCode && (
              <div className="bg-white dark:bg-navy-800 border border-charcoal-100/50 dark:border-navy-700/50 rounded-3xl p-6 shadow-soft">
                <h2 className="text-xl font-semibold text-charcoal-950 dark:text-white mb-4">
                  {t("settings.referralLinks")}
                </h2>
                <p className="text-sm text-charcoal-600 dark:text-gray-400 mb-6">
                  {t("settings.referralLinksDescription")}
                </p>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 bg-charcoal-50 dark:bg-navy-700/50 border border-charcoal-200 dark:border-navy-600 rounded-xl px-4 py-3">
                      <p className="text-sm font-mono text-charcoal-950 dark:text-white break-all">
                        {getReferralLink()}
                      </p>
                    </div>
                    <button
