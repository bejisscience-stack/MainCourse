...
export default function LecturerDashboard() {
  ...
  const handleOpenBundleModal = (bundle?: any) => {
    if (bundle) {
      ...
      const courseIds =
        bundle.course_bundle_items?.map((item: any) => item.course_id) || [];
  ...
  const handleBundleSubmit = async (e: React.FormEvent) => {
    ...
    const bundleOriginalPriceValue = bundleFormData.original_price
      ? parseFloat(bundleFormData.original_price)
      : null;
    ...
    if (
      bundleOriginalPriceValue !== null &&
      (isNaN(bundleOriginalPriceValue) || bundleOriginalPriceValue < 0)
    ) {
      ...
      try {
        ...
        setFormData({
          ...
          referral_commission_percentage:
            courseData.referral_commission_percentage?.toString() || "0",
        });
      } catch (err) {
        ...
        setFormData({
          ...
          referral_commission_percentage:
            (course as any).referral_commission_percentage?.toString() || "0",
        });
      }
    } else {
  ...
  const uploadFile = async (
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () =>
            reject(
              new Error(
                "Upload timeout: File is too large or connection is slow. Please try a smaller file or check your internet connection.",
              ),
            ),
          5 * 60 * 1000,
        );
      ...
      if (uploadError) {
        ...
        throw new Error(
          uploadError.message ||
            "Upload failed. Please check your file size and try again.",
        );
      }
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      const courseData = {
        ...
        lecturer_id: user?.id,
        referral_commission_percentage:
          parseInt(formData.referral_commission_percentage) || 0,
      };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <button
                ...
                title={
                  safeCourses.length < 2
                    ? t("lecturerDashboard.needTwoCourses")
                    : ""
                }
              >
                <svg
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  <p className="font-semibold">
                    {t("lecturerDashboard.errorLoadingDashboard")}
                  </p>
            ...
            <div className="mb-12">
              ...
              {bundlesLoading ? (
                <div className="text-center py-12 bg-white dark:bg-navy-800 border border-charcoal-100/50 dark:border-navy-700/50 rounded-3xl">
                  ...
                  <p className="mt-4 text-charcoal-600 dark:text-gray-400">
                    {t("lecturerDashboard.loadingBundles")}
                  </p>
                </div>
              ) : bundles.length === 0 ? (
                <div className="text-center py-12 bg-white dark:bg-navy-800 border border-charcoal-100/50 dark:border-navy-700/50 rounded-3xl shadow-soft">
                  <p className="text-purple-700 dark:text-purple-300 text-lg mb-4">
                    {t("lecturerDashboard.noBundlesYet")}
                  </p>
                  <button
                    onClick={() => handleOpenBundleModal()}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                  {bundles.map((bundle) => {
                    const bundleCourses =
                      bundle.course_bundle_items?.map(
                        (item: any) => item.courses,
                      ) || [];
                    const totalOriginalPrice = bundleCourses.reduce(
                    ...
                    return (
                      <div
                        key={bundle.id}
                        className="bg-white dark:bg-navy-800 border border-charcoal-100/50 dark:border-navy-700/50 rounded-3xl p-6 shadow-soft hover:shadow-md transition-all duration-200"
                      >
                        <div className="mb-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase tracking-wide">
                              {t("lecturerDashboard.bundle")}
                            </span>
                            {!bundle.is_active && (
                              <span className="text-xs bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded-full">
                                {t("lecturerDashboard.inactive")}
                              </span>
                            )}
                          </div>
                          ...
                          {bundle.description && (
                            <p className="text-sm text-charcoal-600 dark:text-gray-400 line-clamp-2">
                              {bundle.description}
                            </p>
                          )}
                        </div>
                        <div className="mb-4">
                          <p className="text-xs text-charcoal-500 dark:text-gray-500 mb-1">
                            {t("lecturerDashboard.includesCourses", {
                          ...
                          <ul className="text-sm text-charcoal-700 dark:text-gray-300 space-y-1">
                            {bundleCourses
                              ...
                              .map((course: any, idx: number) => (
                                <li key={idx} className="flex items-center">
                                  <svg
                                    className="w-4 h-4 mr-1 text-purple-600 dark:text-purple-400"
                                    fill="currentColor"
                                    viewBox="0 0 20 20"
                                  >
                                    <path
                            ...
                            {bundleCourses.length > 3 && (
                              <li className="text-xs text-charcoal-500 dark:text-gray-500">
                                +{bundleCourses.length - 3} more
                              </li>
                            )}
                          </ul>
                        </div>
                        <div className="flex items-center justify-between mb-4 pt-4 border-t border-charcoal-200 dark:border-navy-600">
                          <div>
                            <p className="text-xs text-charcoal-500 dark:text-gray-500">
                              {t("lecturerDashboard.bundlePrice")}
                            </p>
          ...
          <div className="mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-charcoal-950 dark:text-white mb-6">
              {t("lecturerDashboard.individualCourses")}
            </h2>
          </div>
                ...
                <div className="p-6 border-b border-charcoal-100 dark:border-navy-700 flex-shrink-0">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-charcoal-950 dark:text-white">
                      {editingCourse
                        ? t("lecturerDashboard.editCourse")
                        : t("lecturerDashboard.createNewCourse")}
                    </h2>
                    <button
                    ...
                    {[
                      ...
                      <div key={step.num} className="flex items-center flex-1">
                        ...
                        >
                          ...
                          <span
                            className={`text-xs font-medium ${currentStep === step.num ? "text-emerald-600 dark:text-emerald-400" : "text-charcoal-400 dark:text-gray-500"}`}
                          >
                            {step.label}
                          </span>
                        </button>
                        {idx < 3 && (
                          <div
                            className={`flex-1 h-0.5 mx-2 ${currentStep > step.num ? "bg-emerald-500" : "bg-charcoal-200 dark:bg-navy-600"}`}
                          />
                        )}
                      </div>
                    ))}
                    ...
                    <div className="space-y-5 animate-in fade-in duration-200">
                      <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/30 rounded-2xl p-4 mb-6">
                        <p className="text-sm text-emerald-700 dark:text-emerald-400">
                          Let&apos;s start with the basics. Give your course a
                        ...
                        <div className="relative">
                          ...
                          >
                            <option value="Editing">
                              {t("courses.filterEditing")}
                            </option>
                            <option value="Content Creation">
                              {t("courses.filterContentCreation")}
                            </option>
                            <option value="Website Creation">
                              {t("courses.filterWebsiteCreation")}
                            </option>
                          </select>
                          <svg
                    ...
                    <div className="space-y-5 animate-in fade-in duration-200">
                      <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800/30 rounded-2xl p-4 mb-6">
                        <p className="text-sm text-purple-700 dark:text-purple-400">
                          Set your pricing and referral commission to maximize
                          your earnings.
                        </p>
                      </div>
                      ...
                      <div className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-2xl border border-purple-200/50 dark:border-purple-800/30">
                        <label className="block text-sm font-semibold text-charcoal-700 dark:text-gray-300 mb-3">
                          {t("lecturerDashboard.referralCommissionLabel") ||
                            "Referral Commission"}
                        </label>
                        <div className="flex items-center gap-4">
                          <input
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                referral_commission_percentage: e.target.value,
                              })
                            }
                    ...
                    <div className="space-y-5 animate-in fade-in duration-200">
                      <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/30 rounded-2xl p-4 mb-6">
                        <p className="text-sm text-blue-700 dark:text-blue-400">
                          Add an intro video and thumbnail to make your course
                          stand out.
                        </p>
                      </div>
                      {/* Video Upload */}
                      <div>
                        ...
                        <label className="cursor-pointer block">
                          ...
                          />
                          <div
                            className={`w-full px-6 py-8 border-2 border-dashed rounded-2xl text-center transition-all ${
                          ...
                          >
                            {isUploading && videoUploadProgress > 0 ? (
                              <div className="space-y-3">
                                <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500"></div>
                                <p className="text-sm text-charcoal-700 dark:text-gray-300 font-medium">
                              ...
                              <>
                                <svg
                                  className="w-10 h-10 mx-auto mb-3 text-charcoal-400 dark:text-gray-500"
                                  fill="none"
                                  stroke="currentColor"
                                  viewBox="0 0 24 24"
                                >
                                ...
                                <p className="text-sm font-medium text-charcoal-700 dark:text-gray-300">
                        ...
                        <div className="mt-2">
                          <p className="text-xs text-charcoal-500 dark:text-gray-400 mb-1">
                    ...
                    <div className="space-y-5 animate-in fade-in duration-200">
                      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4 mb-6">
                        <p className="text-sm text-amber-700 dark:text-amber-400">
                          Review your course details and add author information
                          before {editingCourse ? "saving changes" : "creating"}
                          .
                        </p>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-5 bg-gradient-to-br from-charcoal-50 to-charcoal-100 dark:from-navy-700/50 dark:to-navy-800/50 rounded-2xl border border-charcoal-200 dark:border-navy-600">
                        <h4 className="text-sm font-bold text-charcoal-700 dark:text-gray-300 mb-4">
                          Course Summary
                        </h4>
                        <div className="space-y-3 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-charcoal-900 dark:text-white truncate ml-4 max-w-[200px]">
                              {formData.title || "-"}
                            </span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span
                              className={`font-medium ${formData.intro_video_url ? "text-emerald-600" : "text-charcoal-400"}`}
                            >
                              {formData.intro_video_url
                                ? "Uploaded"
                                : "Not uploaded"}
                          ...
                          <div className="flex justify-between">
                            ...
                            <span
                              className={`font-medium ${formData.thumbnail_url ? "text-emerald-600" : "text-charcoal-400"}`}
                            >
                              {formData.thumbnail_url
                                ? "Uploaded"
                                : "Not uploaded"}
                    ...
                    {currentStep < totalSteps ? (
                      ...
                      >
                        {isUploading ? (
                          ...
                          <span>
                            {editingCourse
                              ? t("lecturerDashboard.updateCourse")
                              : t("lecturerDashboard.createCourse")}
                          </span>
                        )}
                      </button>
              ...
              >
                <div className="p-6">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-charcoal-950 dark:text-white">
                      {editingBundle
                        ? t("lecturerDashboard.editBundle")
                        : t("lecturerDashboard.createCourseBundle")}
                    </h2>
                    <button
                  ...
                  <form onSubmit={handleBundleSubmit} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3 bg-white dark:bg-navy-700/50 border border-charcoal-200 dark:border-navy-600 text-charcoal-950 dark:text-white placeholder-charcoal-400 dark:placeholder-gray-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 focus:border-transparent"
                        placeholder={t(
                          "lecturerDashboard.bundleTitlePlaceholder",
                        )}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-3 bg-white dark:bg-navy-700/50 border border-charcoal-200 dark:border-navy-600 text-charcoal-950 dark:text-white placeholder-charcoal-400 dark:placeholder-gray-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 focus:border-transparent"
                        placeholder={t(
                          "lecturerDashboard.bundleDescriptionPlaceholder",
                        )}
                      />
                    </div>
                      ...
                      <div className="border border-charcoal-200 dark:border-navy-600 rounded-xl p-4 max-h-64 overflow-y-auto bg-charcoal-50/50 dark:bg-navy-700/30">
                        {safeCourses.length === 0 ? (
                          <p className="text-sm text-charcoal-500 dark:text-gray-400">
                            {t("lecturerDashboard.noCoursesAvailable")}
                          </p>
                        ) : (
                          <div className="space-y-2">
                            {safeCourses.map((course) => (
                    ...
                    <div className="flex gap-4 pt-4">
                      ...
                      >
                        {editingBundle
                          ? t("lecturerDashboard.updateBundle")
                          : t("lecturerDashboard.createBundle")}
                      </button>
                      <button
                        type="button"
