...
export async function POST(
  ...
  try {
    ...
    if (verifyError) {
      ...
      console.log(
        "[Approve API] Approval successful, verified status:",
      ...
      if (updatedRequest?.user_id) {
        const courseTitle =
          (updatedRequest.courses as { title?: string } | null)?.title ||
          "Unknown Course";
        ...
        try {
          ...
          if (notificationError) {
            console.error(
              "[Approve API] Error creating notification:",
              notificationError,
            );
          } else {
            console.log(
              "[Approve API] Notification created for user:",
              updatedRequest.user_id,
            );
          }
        } catch (notifError) {
          console.error(
            "[Approve API] Exception creating notification:",
            notifError,
          );
        }
    ...
    console.error(
      "Error in POST /api/admin/enrollment-requests/[id]/approve:",
      error,
    );
    return NextResponse.json({ error: "An error occurred" }, { status: 500 });
  }
