...
async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.error(
        `[Admin API ${requestId}] Attempt ${attempt + 1}/${maxRetries} failed:`,
        error.message,
      );
...
async function checkAdmin(
  ...
  try {
    ...
    console.error(
      `[Admin API ${requestId}] Exception checking admin after retries:`,
      err,
    );
    return false;
  }
...
export async function GET(request: NextRequest) {
  ...
  console.log(
    `[Admin API ${requestId}] Request started at ${new Date().toISOString()}`,
  );
  const hasServiceRoleKey = !!process.env.SUPABASE_SERVICE_ROLE_KEY;
  console.log(
    `[Admin API ${requestId}] Service role key present: ${hasServiceRoleKey}`,
  );
  ...
  try {
    ...
    if (!token) {
      console.log(
        `[Admin API ${requestId}] Missing or invalid authorization header`,
      );
    ...
    if (userError || !user) {
      console.log(
        `[Admin API ${requestId}] Token verification failed:`,
        userError?.message,
      );
      return NextResponse.json(
        { error: "Unauthorized", requestId },
    ...
    console.log(
      `[Admin API ${requestId}] Fetching requests, filter:`,
      filterStatus || "all",
    );
    ...
    try {
      console.log(
        "[Admin API] Using RPC function get_enrollment_requests_admin",
      );
      const serviceSupabase = createServiceRoleClient(token);

      // First, verify database count to detect caching issues
      const { data: countData, error: countError } = await serviceSupabase.rpc(
        "check_is_admin",
        { user_id: user.id },
      ); // This also warms up the connection
      ...
      if (rpcError) {
        ...
        let queryBuilder = serviceSupabase
          ...
          .select(
            "id, user_id, course_id, status, created_at, updated_at, reviewed_by, reviewed_at, payment_screenshots, referral_code",
          )
      ...
      if (!dbCountError && dbCount !== null) {
        console.log(
          `[Admin API] DB total count: ${dbCount}, Query returned: ${requests.length}`,
        );
        if (dbCount !== requests.length && !filterStatus) {
          console.warn(
            `[Admin API] WARNING: Count mismatch! DB has ${dbCount} records but query returned ${requests.length}`,
          );
        }
      }
      ...
      console.log(
        "[Admin API] Request statuses from DB:",
        requests.map((r: any) => ({
          id: r.id,
          status: r.status,
          updated_at: r.updated_at,
    ...
    const courseIds = [
      ...new Set(requests.map((r) => r.course_id).filter(Boolean)),
    ];
    ...
    try {
      if (userIds.length > 0) {
        ...
        if (!profilesError && profilesData) {
          ...
          console.error(
            "[Admin API] Service role profiles error:",
            profilesError,
          );
        }
      }
    ...
    try {
      if (courseIds.length > 0) {
        ...
        if (!coursesError && coursesData) {
          ...
          console.error(
            "[Admin API] Service role courses error:",
            coursesError,
          );
        }
      }
    ...
    try {
      if (referralCodes.length > 0) {
        ...
        if (!referrerError && referrerData) {
          ...
          console.error(
            "[Admin API] Service role referrer profiles error:",
            referrerError,
          );
        }
      }
    ...
    const referrerMap = new Map(
      referrerProfiles.map((p) => [p.referral_code, p]),
    );
    ...
    const requestsWithRelations = requests.map((request) => ({
      ...
      referrer: request.referral_code
        ? referrerMap.get(request.referral_code) || null
        : null,
    }));

    console.log(
      "[Admin API] Returning",
      requestsWithRelations.length,
      "requests with relations",
    );
    console.log(
      "[Admin API] Final statuses being returned:",
      requestsWithRelations.map((r) => ({
        id: r.id,
        status: r.status,
        updated_at: r.updated_at,
      })),
