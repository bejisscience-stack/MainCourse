...
export async function GET(request: NextRequest) {
  console.log(
    `[Admin Bundle API] Request started at ${new Date().toISOString()}`,
  );
  const hasServiceRoleKey = !!process.env.SUPABASE_SERVICE_ROLE_KEY;
  console.log(
    `[Admin Bundle API] Service role key present: ${hasServiceRoleKey}`,
  );
  ...
  try {
    ...
    console.log(
      "[Admin Bundle API] Fetching bundle requests, filter:",
      filterStatus || "all",
    );
    ...
    try {
      console.log(
        "[Admin Bundle API] Using RPC function get_bundle_enrollment_requests_admin",
      );
      ...
      const { data: rpcData, error: rpcError } = await serviceSupabase.rpc(
        "get_bundle_enrollment_requests_admin",
        {
      ...
      if (rpcError) {
        ...
        let queryBuilder = serviceSupabase
          ...
          .select(
            "id, user_id, bundle_id, status, created_at, updated_at, reviewed_by, reviewed_at, payment_screenshots",
          )
        ...
        requests = rpcData || [];
        console.log(
          "[Admin Bundle API] RPC succeeded, found",
          requests.length,
          "requests",
        );
      }
      ...
      if (requests.length > 0) {
        console.log(
          "[Admin Bundle API] Raw bundle DB data (first 3):",
      ...
      if (!dbCountError && dbCount !== null) {
        console.log(
          `[Admin Bundle API] DB total count: ${dbCount}, Query returned: ${requests.length}`,
        );
        if (dbCount !== requests.length && !filterStatus) {
          console.warn(
            `[Admin Bundle API] WARNING: Count mismatch! DB has ${dbCount} records but query returned ${requests.length}`,
          );
        }
      }

      console.log(
        "[Admin Bundle API] Request statuses:",
        requests.map((r: any) => ({ id: r.id, status: r.status })),
      );
    } catch (err: any) {
    ...
    if (requestsError && requests.length === 0) {
      console.error(
        "[Admin Bundle API] Failed to fetch bundle requests:",
        requestsError,
      );
      return NextResponse.json(
        {
    ...
    const bundleIds = [
      ...new Set(requests.map((r) => r.bundle_id).filter(Boolean)),
    ];
    ...
    try {
      if (bundleIds.length > 0) {
        ...
        const { data: bundlesData, error: bundlesError } =
          await createServiceRoleClient(token)
            .from("course_bundles")
    ...
    const requestsWithRelations = requests.map((request) => {
      ...
      if (typeof paymentScreenshots === "string") {
        try {
          ...
          console.warn(
            "[Admin Bundle API] Failed to parse payment_screenshots for request",
            request.id,
            e,
          );
          paymentScreenshots = [];
    ...
    console.log(
      "[Admin Bundle API] Returning",
      requestsWithRelations.length,
      "bundle requests with relations",
    );
    console.log(
      "[Admin Bundle API] Final bundle statuses being returned:",
