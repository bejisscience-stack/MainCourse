...
async function checkAdmin(supabase: any, userId: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error(
        "[Admin Withdrawals API] Error checking admin status:",
        error,
      );
      return false;
    }
...
export async function GET(request: NextRequest) {
  console.log(
    `[Admin Withdrawals API] Request started at ${new Date().toISOString()}`,
  );
  const hasServiceRoleKey = !!process.env.SUPABASE_SERVICE_ROLE_KEY;
  console.log(
    `[Admin Withdrawals API] Service role key present: ${hasServiceRoleKey}`,
  );
  ...
  try {
    ...
    console.log(
      "[Admin Withdrawals API] Fetching requests, filter:",
      filterStatus || "all",
    );
    ...
    try {
      console.log(
        "[Admin Withdrawals API] Using RPC function get_withdrawal_requests_admin",
      );
      ...
      if (rpcError) {
        ...
        let queryBuilder = serviceSupabase
          ...
          .select(
            "id, user_id, user_type, amount, bank_account_number, status, admin_notes, processed_at, processed_by, created_at, updated_at",
          )
        ...
        requests = rpcData || [];
        console.log(
          "[Admin Withdrawals API] RPC succeeded, found",
          requests.length,
          "requests",
        );
      }
      ...
      if (!dbCountError && dbCount !== null) {
        console.log(
          `[Admin Withdrawals API] DB total count: ${dbCount}, Query returned: ${requests.length}`,
        );
        if (dbCount !== requests.length && !filterStatus) {
          console.warn(
            `[Admin Withdrawals API] WARNING: Count mismatch! DB has ${dbCount} records but query returned ${requests.length}`,
          );
        }
      }

      console.log(
        "[Admin Withdrawals API] Request statuses:",
    ...
    if (requestsError && requests.length === 0) {
      console.error(
        "[Admin Withdrawals API] Failed to fetch requests:",
        requestsError,
      );
    ...
    console.log(
      "[Admin Withdrawals API] Processing",
      requests.length,
      "requests",
    );
    ...
    try {
      if (userIds.length > 0) {
        ...
        if (!profilesError && profilesData) {
          ...
          console.error(
            "[Admin Withdrawals API] Service role profiles error:",
            profilesError,
          );
        }
      }
    ...
    console.log(
      "[Admin Withdrawals API] Returning",
