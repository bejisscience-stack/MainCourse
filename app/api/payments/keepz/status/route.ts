  ...
  try {
    ...
    if (payment.status === 'created' && payment.keepz_order_id) {
      try {
        ...
        const orderStatus = (keepzStatus.orderStatus as string) || (keepzStatus.status as string);
        ...
        if (orderStatus === 'SUCCESS') {
          ...
          const { data: rpcResult, error: rpcError } = await serviceClient.rpc('complete_keepz_payment', {
            p_keepz_order_id: payment.keepz_order_id,
            p_callback_payload: keepzStatus,
          });

          if (rpcError || rpcResult?.success === false) {
            console.error('[Keepz Status] Failed to complete payment via verification:', {
              rpcError,
              rpcResult,
              paymentId,
            });
          } else {
            console.log('[Keepz Status] Payment completed via verification fallback:', { paymentId });
        ...
        } else if (orderStatus === 'FAILED' || orderStatus === 'REJECTED' || orderStatus === 'CANCELLED') {
        ...
        console.error('[Keepz Status] Verification error (non-fatal):', verifyError);
      }
    }
