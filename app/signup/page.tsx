...
function SignUpForm() {
  ...
  const [inAppBrowser, setInAppBrowser] = useState<InAppBrowserResult | null>(
    null,
  );
  ...
  useEffect(() => {
    const validateReferralCode = async () => {
      ...
      // Store redirect URL for post-signup navigation (validated to prevent open redirects)
      const safeRedirect = validateRedirectUrl(redirect);
      if (safeRedirect) {
        setRedirectUrl(safeRedirect);
      }
      ...
      if (ref) {
        ...
        try {
          ...
          if (!result.valid) {
            ...
            setReferralError(
              `Note: Referral code "${normalizedRef}" was not found. You can still sign up, but the referral won't be applied.`,
            );
          ...
          setReferralError(
            `Note: Referral code "${normalizedRef}" could not be validated. You can still sign up, but the referral won't be applied.`,
          );
          setReferralCode("");
        }
      }
        ...
        <div>
          ...
          {referralCode && (
            <div className="mt-4 bg-emerald-50 dark:bg-emerald-500/20 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 px-4 py-3 rounded-lg text-sm text-center">
              {t("auth.referralLinkDetected")?.replace(
                "{{code}}",
                referralCode,
              ) ||
                `You've been referred by someone! Referral code: ${referralCode}`}
            </div>
          )}
          {referralError && (
        ...
        {success ? (
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            {inAppBrowser?.isInApp ? (
              /* In-app browser detected: Google OAuth will fail with disallowed_useragent. */
              <div
                role="note"
                className="rounded-lg border border-amber-300 dark:border-amber-500/40 bg-amber-50 dark:bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-100"
              >
                <p className="font-semibold">{t("auth.inAppBrowserTitle")}</p>
                <p className="mt-1">{t("auth.inAppBrowserBody")}</p>
                {/Android/i.test(
                  typeof navigator !== "undefined" ? navigator.userAgent : "",
                ) ? (
                  <a
                    href="intent://swavleba.ge/signup#Intent;scheme=https;package=com.android.chrome;end"
